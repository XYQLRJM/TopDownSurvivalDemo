...
public class UIMgr : BaseManager<UIMgr>
{
    ...
    private UIMgr()
    {
        ...
        //如果存在UI摄像机则使用摄像机渲染，否则保留Canvas自身的Overlay模式。
        if (uiCamera != null)
            uiCanvas.worldCamera = uiCamera;
        //过场景不移除
        GameObject.DontDestroyOnLoad(uiCanvas.gameObject);

        //找到层级父对象，没有分层节点时直接使用Canvas自身作为父节点。
        bottomLayer = uiCanvas.transform.Find("Bottom") ?? uiCanvas.transform;
        middleLayer = uiCanvas.transform.Find("Middle") ?? uiCanvas.transform;
        topLayer = uiCanvas.transform.Find("Top") ?? uiCanvas.transform;
        systemLayer = uiCanvas.transform.Find("System") ?? uiCanvas.transform;
    ...
    public void ShowPanel<T>(E_UILayer layer = E_UILayer.Middle, UnityAction<T> callBack = null, bool isSync = false) where T:BasePanel
    {
        ...
        //不存在面板时只从 Resources 加载。
        GameObject resourcePanel = ResMgr.Instance.Load<GameObject>($"Prefabs/UI/{panelName}");
        if (resourcePanel != null)
        {
            CreatePanel(resourcePanel, layer, panelName);
            return;
        }

        Debug.LogError($"UI panel not found: Resources/Prefabs/UI/{panelName}.prefab");
        panelDic.Remove(panelName);

        void CreatePanel(GameObject res, E_UILayer targetLayer, string targetPanelName)
        {
            //取出字典中已经占好位置的数据
            PanelInfo<T> panelInfo = panelDic[targetPanelName] as PanelInfo<T>;
            //表示异步加载结束前 就想要隐藏该面板了 
            if(panelInfo.isHide)
            {
                panelDic.Remove(targetPanelName);
                return;
            }

            //层级的处理
            Transform father = GetLayerFather(targetLayer);
